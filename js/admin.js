...
async function loadHygiene() {
    ...
    container.innerHTML = Object.entries(groups).map(([dept, emps]) => {
        const rows = emps.map(emp => {
            ...
            <div style="border-bottom:1px solid #F0F0F0;">
                <div onclick="const b=this.nextElementSibling; const open=b.style.display!=='none'; b.style.display=open?'none':'block'; this.querySelector('.hyg-arrow').textContent=open?'▶':'▼';"
                     style="display:grid; grid-template-columns:1fr 1fr 1fr 5% 5%; gap:0.5rem; align-items:center; padding:0.6rem 0; font-size:0.85rem; cursor:pointer;">
                    <div style="font-weight:600;">${emp.name}</div>
        ...
        <div style="background:white; border-radius:12px; overflow:hidden; padding:0 0.75rem;">
            <div style="display:grid; grid-template-columns:1fr 1fr 1fr 5% 5%; gap:0.5rem; padding:0.4rem 0; border-bottom:2px solid #F0F0F0; font-size:0.7rem; font-weight:700; color:var(--color-text-light);">
